...
namespace pepenet_social {
  ibool post_args::loadArgsFromJson()
  {
    if (!m_schema_valid)
    {
    ...
    {
      m_pseudonym = m_json["post_args"]["pseudonym"].GetString();
    }
    if (m_json["post_args"].HasMember("sk_seed"))
    {
      if (!m_json["post_args"].HasMember("post_pk"))
      {
        m_valid_args = false;
        return FALSE_IBOOL("json field post_pk is required when sk_seed is defined!");
      }
      m_sk_seed = m_json["post_args"]["sk_seed"].GetString();
    }
    if (m_json["post_args"].HasMember("post_pk"))
    {
      m_post_pk = m_json["post_args"]["post_pk"].GetBool();
      if (!m_json["post_args"].HasMember("sk_seed"))
      {
        m_valid_args = false;
    ...
    if (m_json["post_args"].HasMember("tx_ref"))
    {
      const std::string tx_ref_hex = m_json["post_args"]["tx_ref"].GetString();
    ...
    if (m_json["post_args"].HasMember("pepetag"))
    {
      m_pepetag = m_json["post_args"]["pepetag"].GetString();
    }
    if (m_json["post_args"].HasMember("donation_address"))
    {
      m_donation_address = m_json["post_args"]["donation_address"].GetString();
    }
  ...
  void post_args::setSchema()
  {
    m_json_schema_str = R"(
{
  "$id": "post_args schema pepenet hfv2",
  "$schema": "https://json-schema.org/draft/2020-12/schema",
  "title": "pepenet post args",
  "type": "object",
  "properties": {
    "post_args": {
  ...
  ibool post::loadFromSocialArgs(post_args const& args)
  {
    ...
    if (args.m_sk_seed.has_value()) //create sig
    {
      ...
      pepenet_social_protos::post_base* base_ptr = m_proto.release_base(); //get base prt
  ...
  ibool post::validate()
  {
    ...
    {
      return FALSE_IBOOL("failed to verify post: missing sig for peped pk");
    }
    if (m_sig.has_value() && m_pk.has_value()) //verify base
    {
      ...
      {
        return FALSE_IBOOL("failed to serialize post base");
      }
      ...
      {
        return FALSE_IBOOL("failed to verify post: invalid sig");
      }
    }
    m_valid = true;
    return ibool{ true, INFO_NULLOPT };
  ...
  ibool post::validate(const boost::optional<crypto::public_key>& pk)
  {
    m_pk = pk;
