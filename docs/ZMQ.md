# The Current/Future Status of ZMQ in pepenet - defined by monero

## ZMQ Pub/Sub
Client `ZMQ_SUB` sockets must "subscribe" to topics before it receives any data.
This allows filtering on the server side, so network traffic is reduced. pepenet
...
matching supported by both pepenet and ZMQ. The `format`, `context` and `event`
...
The pepenet daemon will ensure that events prefixed by `chain` will be sent in
...
chain instead of the last block. The pepenet daemon also ensures that
...
The pepenet daemon will send a `txpool_add` pub exactly once for each
