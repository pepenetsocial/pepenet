...
is made or until `pepenetd` is shutdown and restarted with only public
connections enabled.

Anonymity networks can also be used with `pepenet-wallet-cli` and
`pepenet-wallet-rpc` - the wallets will connect to a daemon through a proxy. The
...
`--tx-proxy` which tells `pepenetd` the ip/port of a socks proxy provided by a
...
which tells `pepenetd` that ".onion" p2p addresses can be forwarded to a socks
...
`--anonymous-inbound`. This option tells `pepenetd` the inbound address, network
...
which tells `pepenetd` that a max of 25 inbound Tor connections are being
received at address "rveahdfho7wo4b2m.onion:28083" and forwarded to `pepenetd`
localhost port 28083, and a default max I2P connections are being received at
address "cmeua5767mz2q5jsaelk2rxhf67agrwuetaso5dzbenyzwlbkg2q.b32.i2p:5000" and
forwarded to `pepenetd` localhost port 30000.
...
`pepenetd` RPC port - which is independent from the configuration for incoming
...
```
HiddenServiceDir /var/lib/tor/data/pepenet
...
This will store key information in `/var/lib/tor/data/pepenet` and will forward
"Tor port" 28083 to port 28083 of ip 127.0.0.1. The file
`/usr/lib/tor/data/pepenet/hostname` will contain the ".onion" address for use
...
If a user only runs `pepenetd` to send a transaction then quit, this can also
be used by an ISP to link a user to a transaction.

#### Mitigation

Run `pepenetd` as often as possible to conceal when transactions are being sent.
...
`pepenetd` currently selects two outgoing connections every 5 minutes for
...
`pepenetd` will have rotated its selected outgoing connections several times in
...
circuits is observable by the ISP. Instead, `pepenetd` should likely disconnect
