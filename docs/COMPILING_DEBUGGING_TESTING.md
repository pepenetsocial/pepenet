...
The additional artifact of the above call is the `pepenet.cbp` Code::Blocks project file in the build directory.
...
After a successful build, open the `pepenet.cbp` with CB. From the CB's menu bar select the target, that you want debug. Assuming these are unit tests:
