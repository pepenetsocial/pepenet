...
# a main offline pepenetd, does most of the tests
# a restricted RPC pepenetd setup with RPC payment
# two local online pepenetds connected to each other
N_PEPENETDS = 4

# 4 wallets connected to the main offline pepenetd
# 1 wallet connected to the first local online pepenetd
# 1 offline wallet
N_WALLETS = 6
...
pepenetd_base = [builddir + "/bin/pepenetd", "--regtest", "--fixed-difficulty", str(DIFFICULTY), "--no-igd", "--p2p-bind-port", "pepenetd_p2p_port", "--rpc-bind-port", "pepenetd_rpc_port", "--zmq-rpc-bind-port", "pepenetd_zmq_port", "--zmq-pub", "pepenetd_zmq_pub", "--non-interactive", "--disable-dns-checkpoints", "--check-updates", "disabled", "--rpc-ssl", "disabled", "--data-dir", "pepenetd_data_dir", "--log-level", "1"]
pepenetd_extra = [
  ["--offline"],
...
wallet_base = [builddir + "/bin/pepenet-wallet-rpc", "--wallet-dir", WALLET_DIRECTORY, "--rpc-bind-port", "wallet_port", "--disable-rpc-login", "--rpc-ssl", "disabled", "--daemon-ssl", "disabled", "--log-level", "1", "--allow-mismatched-daemon-version"]
wallet_extra = [
  ["--daemon-port", "18180"],
  ["--daemon-port", "18180"],
  ["--daemon-port", "18180"],
...
for i in range(N_PEPENETDS):
  command_lines.append([str(18180+i) if x == "pepenetd_rpc_port" else str(18280+i) if x == "pepenetd_p2p_port" else str(18380+i) if x == "pepenetd_zmq_port" else "tcp://127.0.0.1:" + str(18480+i) if x == "pepenetd_zmq_pub" else builddir + "/functional-tests-directory/pepenetd" + str(i) if x == "pepenetd_data_dir" else x for x in pepenetd_base])
  if i < len(pepenetd_extra):
    command_lines[-1] += pepenetd_extra[i]
  outputs.append(open(FUNCTIONAL_TESTS_DIRECTORY + '/pepenetd' + str(i) + '.log', 'a+'))
