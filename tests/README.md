...
Core tests take longer than any other pepenet tests, due to the high amount of computational work involved in validating core components.
...
To run only pepenet's core tests (after building):
...
To run only pepenet's crypto tests (after building):
...
```bash
pepenetd --regtest --offline --fixed-difficulty 1
```
...
Open the wallet file with `pepenet-wallet-rpc` with RPC port 18083. Finally, start tests by invoking ./blockchain.py or ./speed.py
...
To run only pepenet's hash tests (after building):
...
To run only pepenet's performance tests (after building):
...
To run only pepenet's unit tests (after building):
